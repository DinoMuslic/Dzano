if (typeof SingleGig === "undefined") {
  const SingleGig = (function () {
    ...
    function render() {
      ...
      if (!gigId) {
        $container.html(
          `<div class="alert alert-danger">Invalid gig ID</div>`
        );
        return;
      }
      ...
      $.ajax({
          ...
          const gigHtml = `
            <div class="card shadow border-0">
              <div class="card-body pb-0">
                <div class="d-flex justify-content-between mb-2">
                  <span class="text-uppercase fw-bold text-primary">${
                    gig.category_name || "Uncategorized"
                  }</span>
                  <span class="text-muted">${createdAt}</span>
                </div>

                <h2 class="fw-bold">${gig.title}</h2>
                <p class="text-muted mb-3">by <strong>${
                  gig.user_first_name && gig.user_last_name
                    ? gig.user_first_name + " " + gig.user_last_name
                    : "Anonymous"
                }</strong></p>
              </div>
              ...
              <div class="card-body mt-4">
                ...
                <div class="mb-3">
                  <span class="badge bg-${
                    gig.status === "open" ? "success" : "secondary"
                  }">Status: ${gig.status || "Open"}</span>
                </div>
                ...
                ${
                  ...
                  <div class="mt-3">
                    ${gig.tags
                      ...
                      .map(
                        (tag) =>
                          `<span class="badge bg-info text-dark me-2">${tag.trim()}</span>`
                      )
                      .join("")}
                  </div>`
                    : ""
              ...
              success: function (res) {
                ...
                if (status === "approved") {
                  $applyBtn
                    ...
                    .html(
                      '<i class="fas fa-check-circle me-1"></i> Approved – Contact Owner'
                    )
                    .prop("disabled", true);
                  ...
                  $(document)
                    .off("submit", "#applicationForm")
                    .on("submit", "#applicationForm", function (e) {
                      e.preventDefault();
                      ...
                      $.ajax({
                        ...
                        error: function (xhr) {
                          toastr.error(
                            "Failed to apply: " + xhr.responseText
                          );
                        },
                      });
                    });
              ...
              success: function (res) {
                ...
                if (isFav) {
                  $favBtn
                    ...
                    .addClass("btn-danger")
                    .html(
                      '<i class="fas fa-heart me-1"></i> Remove Favorite'
                    )
                    .data("favorited", true);
                } else {
                  $favBtn
        ...
        error: function (xhr) {
          ...
          $container.html(
            `<div class="alert alert-danger">Error loading gig: ${xhr.responseText}</div>`
          );
        },
      });
    }
